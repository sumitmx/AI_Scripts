...
class AppCleanupStatus(BaseModel):
    app_name: str
...
class BulkUninstallResult(BaseModel):
    overall_status: str  # "Success", "Partial Success", "Failed"
    status: Dict[str, str]
    status_message: Dict[str, str]
    logs: Dict[str, str]

def run_bulk_uninstaller() -> BulkUninstallResult:
    logger = Logger()
    ...
    try:
        raw_input = input_provider.get_value("Applications_1750856640900")
        input_data = CleanupInput(applications=eval(raw_input))
        app_names = [name.lower().replace(" ", "") for name in input_data.applications]

        for app in app_names:
            cleaner = cleaners.get(app)
            if cleaner:
                log_text, action, status, message = cleaner.clean()
                ...
                message_by_app[app] = message
            else:
                logs_by_app[app] = f"No cleaner available for {app}"
                status_by_app[app] = f"{app} : Could not uninstall; no cleaner"
                message_by_app[app] = "See logs for details."

    except Exception as e:
        logs_by_app["BulkUninstaller"] = str(e)
        status_by_app["BulkUninstaller"] = "failed"
        message_by_app["BulkUninstaller"] = f"Exception occurred: {e}"

    status_values = list(status_by_app.values())
    if all("uninstalled successfully" in s for s in status_values):
        overall_status = "Success"
    elif any("uninstalled successfully" in s for s in status_values):
    ...
    logger.result_data(result.model_dump())
    return result
...
#     try:
#         input_list = input_provider.get_value("Applications_1750856640900")
#         logger.info("Staring with input list")

#         app_names_raw = eval(input_list)
#         if not isinstance(app_names_raw, list):
#             raise ValueError("Expected a list of app names.")
        
#         logger.info("Staring with cleanup")
...
#         logger.info(status_by_app)
#         logger.info(message_by_app)
#         logger.info(logs_by_app)
