...
logger.info("Starting malware uninstaller script...")
...
class BulkUninstallerResult(BaseModel):
    ...
    Attributes:
        ...
        ticket_notes: Human-readable summary for ticket notes
    """
...
UNINSTALLER_SCRIPTS: Dict[str, UninstallerScript] = {
    "PCAppStore": UninstallerScript(
        name="PCAppStore",
        log_path=Path(r"C:\Temp\PCAppStore_Removal.log"),
        powershell_code=r"""# PCAppStore Complete Removal Script
...
$UserPathTemplates = @(
    "\PCAppStore",
    "\AppData\Local\Programs\PCAppStore",
    "\AppData\Local\PCAppStore",
    "\AppData\Roaming\PCAppStore"
)
...
$UserSoftwareRegPaths = @("\software\PCAppStore")

$TaskNamePatterns = @("PCAppStore", "PC App Store")

Write-Host "$AppName Complete Removal Script" -ForegroundColor Cyan
...
function Write-Log {
    ...
    $LogFile = "C:\PCAppStore_Removal.log"
...
function Stop-Processes {
    ...
    try {
        $userPaths = @()
        foreach ($user in Get-ChildItem 'C:\Users' -Directory -ErrorAction Stop) {
            foreach ($template in $UserPathTemplates) {
                $resolvedPath = Join-Path $user.FullName $template.TrimStart('\')
                $userPaths += $resolvedPath
            }
        }
    }
    catch {
        Write-Log "Error accessing user directories: $($_.Exception.Message)" -Level "ERROR"
        Write-Host "Error accessing user directories: $($_.Exception.Message)" -ForegroundColor Red
        $userPaths = @()
    }

    $allPaths = $userPaths + $SystemPaths
    $pathsNorm = $allPaths | ForEach-Object {
        $_.TrimEnd('\').ToLower()
    } | Select-Object -Unique

    Write-Log "Checking $($pathsNorm.Count) normalized paths for $AppName processes..." -Level "INFO"

    if ($ShowPaths) {
        Write-Log "Paths being checked:" -Level "INFO"
        $pathsNorm | ForEach-Object { Write-Log "  $_" -Level "INFO" }
    }

    Write-Log "Searching for $AppName processes..." -Level "INFO"
    Write-Host "Searching for $AppName processes..." -ForegroundColor Cyan
    ...
    foreach ($proc in $allProcesses) {
        if ($proc.ExecutablePath) {
            $parentDir = (Split-Path $proc.ExecutablePath -Parent).ToLower()
            $matchedPath = $pathsNorm | Where-Object { $parentDir -like "*$($_)*" }

            if ($matchedPath.Count -gt 0) {
                Write-Log "MATCH FOUND: $($proc.Name) (PID: $($proc.ProcessId)) at $($proc.ExecutablePath)" -Level "SUCCESS"
                Write-Host "MATCH FOUND: $($proc.Name) (PID: $($proc.ProcessId)) at $($proc.ExecutablePath)" -ForegroundColor Green
                $processes += $proc
            }
        }
    }
    ...
    if ($processes.Count -gt 0) {
        Write-Log "Found $($processes.Count) $AppName process(es) to terminate" -Level "INFO"
        ...
        foreach ($proc in $processes) {
            Write-Log "Attempting to kill: $($proc.Name) (PID: $($proc.ProcessId))" -Level "INFO"
            Write-Host "Attempting to kill: $($proc.Name) (PID: $($proc.ProcessId))" -ForegroundColor Cyan
            ...
            try {
                ...
                if (-not (Get-Process -Id $proc.ProcessId -ErrorAction SilentlyContinue)) {
                    Write-Log "Successfully killed PID $($proc.ProcessId) using Stop-Process" -Level "SUCCESS"
                    Write-Host "(SUCCESS) Killed with Stop-Process" -ForegroundColor Green
                    $killCount++
                    $terminated = $true
                }
            } catch {
                Write-Log "Stop-Process failed for PID $($proc.ProcessId): $($_.Exception.Message)" -Level "WARN"
                Write-Host "Stop-Process failed: $($_.Exception.Message)" -ForegroundColor Yellow
            }

            if (-not $terminated) {
                try {
                    Start-Process -FilePath "taskkill" -ArgumentList "/PID", $proc.ProcessId, "/F", "/T" -Wait -WindowStyle Hidden -ErrorAction Stop
                    Start-Sleep -Seconds 2
                    if (-not (Get-Process -Id $proc.ProcessId -ErrorAction SilentlyContinue)) {
                        Write-Log "Successfully killed PID $($proc.ProcessId) using taskkill" -Level "SUCCESS"
                        Write-Host "(SUCCESS) Killed with taskkill" -ForegroundColor Green
                        $killCount++
                        $terminated = $true
                    }
                } catch {
                    Write-Log "taskkill failed for PID $($proc.ProcessId): $($_.Exception.Message)" -Level "WARN"
                    Write-Host "taskkill failed: $($_.Exception.Message)" -ForegroundColor Yellow
                }
            }

            if (-not $terminated) {
                Write-Log "Failed to terminate PID $($proc.ProcessId) with all methods" -Level "ERROR"
                Write-Host "(FAILED) Could not terminate PID $($proc.ProcessId)" -ForegroundColor Red
                $failCount++
            }
        }

        Write-Log "Termination Summary: $killCount successful, $failCount failed" -Level "INFO"
        Write-Host "Summary: $killCount success, $failCount failed" -ForegroundColor White

        if ($failCount -eq 0) {
            Write-Log "All $AppName processes terminated successfully!" -Level "SUCCESS"
            Write-Host "All processes terminated!" -ForegroundColor Green
            return $true
        } else {
            Write-Log "Some $AppName processes could not be terminated" -Level "ERROR"
            Write-Host "Some processes could not be terminated." -ForegroundColor Red
            return $false
        }
    } else {
        Write-Log "No $AppName processes found running from target directories" -Level "INFO"
        Write-Host "No processes matched target directories." -ForegroundColor Cyan
        return $true
    }
}
...
function Remove-Folders {
    ...
    foreach ($u in $UserProfiles) {
        foreach ($tpl in $UserPathTemplates) {
            $AllPaths += Join-Path -Path "C:\Users\$u" -ChildPath $tpl
        }
    }
    ...
    foreach ($path in $AllPaths) {
        try {
            ...
            if (Test-Path $expandedPath) {
				...
				Write-Host "Removing: $expandedPath"
                ...
                if (-not (Test-Path $expandedPath)) {
					...
					Write-Host "Successfully removed: $expandedPath" -Level "SUCCESS"
                } else {
                    Write-Log "Failed to remove: $expandedPath" -Level "ERROR"
					Write-Host "Failed to remove: $expandedPath" -Level "ERROR"
                }
            } else {
			...
			Write-Host "Error removing '$path': $_" -Level "ERROR"
        }
    }
}
...
function Remove-UserRegistryPaths {

    foreach ($registry_hive in (Get-ChildItem registry::hkey_users -ErrorAction SilentlyContinue)) {
        foreach ($UserSoftwareRegPaths in $reg_paths) {
            $path = $registry_hive.PSPath + $UserSoftwareRegPaths

            if (Test-Path $path) {
                try {
                    Remove-Item -Path $path -Recurse -Force -ErrorAction Stop
                    Write-Log "$path has been removed." -Level "SUCCESS"
                    Write-Host "$path has been removed." -ForegroundColor Green
                } catch {
...
function Remove-RunKeys {
    ...
    # Build the list of full paths to search for
    $searchPaths = @()
    $UserProfiles = Get-ChildItem 'C:\Users' | Where-Object { $_.PSIsContainer } | Select-Object -ExpandProperty Name

    foreach ($u in $UserProfiles) {
        foreach ($tpl in $UserPathTemplates) {
            $searchPaths += Join-Path "C:\Users\$u" $tpl
        }
    }
    $searchPaths += $SystemPaths

    # Scan all HKEY_USERS Run keys
    foreach ($registry_hive in (Get-ChildItem registry::hkey_users -ErrorAction SilentlyContinue)) {
        $runPath = "$($registry_hive.PSPath)\Software\Microsoft\Windows\CurrentVersion\Run"

        if (Test-Path $runPath) {
            $regKey = Get-Item $runPath -ErrorAction SilentlyContinue
            foreach ($propName in $regKey.GetValueNames()) {
                $propData = $regKey.GetValue($propName)

                foreach ($targetPath in $searchPaths) {
                    if ($propData -like "*$targetPath*") {
                        try {
                            Remove-ItemProperty -Path $runPath -Name $propName -ErrorAction Stop
                            Write-Log "$runPath\$propName matched path '$targetPath' and was removed." -Level "SUCCESS"
							Write-Host "$runPath\$propName matched path '$targetPath' and was removed." -ForegroundColor Green
                        } catch {
                            Write-Log "Failed to remove $runPath\$propName`: $_" -Level "ERROR"
							Write-Host "Failed to remove $runPath\$propName`: $_" -ForegroundColor Red
                        }
                        break
                    }
                }
            ...
            Write-Log "Run key not found under $($registry_hive.PSPath)" -Level "INFO"
        }
    }
}
...
function Remove-UninstallKeys {
    ...
    try {
        ...
        foreach ($UserHive in $UserHives) {
            $UninstallPath = "$($UserHive.PSPath)\Software\Microsoft\Windows\CurrentVersion\Uninstall"

            if (Test-Path $UninstallPath) {
                $UserKeys = Get-ChildItem -Path $UninstallPath -ErrorAction SilentlyContinue
                ...
                foreach ($key in $UserKeys) {
                    ...
                    foreach ($path in $searchPaths) {
                        if (
                            ($InstallLocation -and $InstallLocation -like "*$path*") -or
                            ($UninstallString -and $UninstallString -like "*$path*")
                        ) {
                            $script:UninstallKeys += $key
                            Write-Log "Found matching uninstall key: $DisplayName at $($key.PSPath)" -Level INFO
                            break
                        }
                    }
                }
        ...
        # Remove matching uninstall keys
    ...
    Write-Log "Uninstall registry cleanup completed. Total matched keys: $($UninstallKeys.Count)"
}

function Remove-ScheduledTasks {
    try {
        # Find matching tasks using external pattern list
        $TasksToRemove = Get-ScheduledTask | Where-Object {
            foreach ($pattern in $TaskNamePatterns) {
                if ($_.TaskName -match "(?i)$pattern") { return $true }
            }
            return $false
        }
        ...
        if ($TasksToRemove.Count -eq 0) {
            ...
            Write-Log "Found $($TasksToRemove.Count) matching scheduled task(s):"
            Write-Host "Found $($TasksToRemove.Count) matching scheduled task(s):" -ForegroundColor Yellow
            ...
            foreach ($Task in $TasksToRemove) {
                ...
                try {
                    ...
                    Write-Host "    (FAILED) Failed to remove: $_" -ForegroundColor Red
                }
            }
        }
    } catch {
...
# Step 1: Collect SHA1 Hashes

Write-Log "================================================================"
Write-Log "Starting $AppName SHA1 Extractor"
Write-Log "================================================================"
Write-Host "================================================================"
Write-Host "Starting $AppName SHA1 Extractor"
Write-Host "================================================================"
...
Write-Log "$AppName SHA1 Extracter Has Finished. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName SHA1 Extractor Has Finished. Exit code: $exitCode"
Write-Host "================================================================"


#Step 2: Attempt Graceful Uninstall
Write-Log "================================================================"
Write-Log "$AppName Graceful Uninstaller Has Started."
Write-Log "================================================================"
Write-Host "================================================================"
Write-Host "$AppName Graceful Uninstaller Has Started."
Write-Host "================================================================"

# Check registry uninstall locations
    ...
    try {
        ...
        $FoundKeys = Get-ChildItem -Path $KeyPath -ErrorAction SilentlyContinue | 
                    Where-Object { 
                        ...
                        $displayName -and (
                            $displayName -match "(?i)PCAppStore" -or 
                            $displayName -match "(?i)PC App Store"
                        )
                    }
...
if ($UninstallKeyFound) {
    ...
    if ($UninstallSuccess) {
        Write-Log "At least one graceful uninstall of $AppName was successful!" -Level "SUCCESS"
        Write-Host "(SUCCESS) Graceful uninstall completed successfully for $AppName!" -ForegroundColor Green
    } else {
        Write-Log "All graceful uninstall attempts FAILED for $AppName. Proceeding with forced cleanup." -Level "ERROR"
        Write-Host "(FAILED) Graceful uninstall FAILED for $AppName. Proceeding with forced cleanup." -ForegroundColor Red
    }
} else {
    Write-Log "No $AppName uninstall entries found in registry." -Level "WARN"
    Write-Host "! $AppName not found in uninstall registry." -ForegroundColor Yellow
    Write-Host "  Proceeding directly to forced cleanup..." -ForegroundColor Yellow
}

Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Graceful Uninstaller Has Finished. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Graceful Uninstaller Has Finished. Exit code: $exitCode"
Write-Host "================================================================"

#Step 3: Detect & Kill All Matching Processes

Write-Log "================================================================"
Write-Log "Starting $AppName Process Terminator"
Write-Log "================================================================"
Write-Host "================================================================"
Write-Host "Starting $AppName Process Terminator"
Write-Host "================================================================"
...
if (-not $isAdmin) {
    Write-Log "WARNING: Not running as administrator. Process termination may fail." -Level "WARN"
    Write-Host "WARNING: Not running as administrator. Process termination may fail." -Level "WARN"
} else {
...
# Call the function
try {
    $result = Stop-Processes -ShowPaths
    
    if ($result) {
        Write-Log "Function completed successfully" -Level "SUCCESS"
        $exitCode = 0
    } else {
        Write-Log "Function completed with errors" -Level "ERROR"
        $exitCode = 1
    }
}
catch {
    Write-Log "Function execution failed: $($_.Exception.Message)" -Level "ERROR"
    $exitCode = 1
}

Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Process Terminator Has Finished. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Process Terminator Has Finished. Exit code: $exitCode"
Write-Host "================================================================"

#Step 4: Remove PCApp Store Folders
Write-Log "================================================================"
Write-Log "Starting $AppName Folder Annihilator"
Write-Log "================================================================"
Write-Host "================================================================"
Write-Host "Starting $AppName Folder Annihilator"
Write-Host "================================================================"
Remove-Folders
Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Folder Annihilator Has Finished Annihilating. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Folder Annihilator Has Finished Annihilating. Exit code: $exitCode"
Write-Host "================================================================"

#Step 6: Removing registry properties..."

Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Registry Entry Annihilator Has Started. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Registry Entry Annihilator Has Started. Exit code: $exitCode"
Write-Host "================================================================"

Remove-UserRegistryPaths
Remove-RunKeys
Remove-UninstallKeys

Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Registry Entry Annihilator Has Finished. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Registry Entry Annihilator Has Finished. Exit code: $exitCode"
Write-Host "================================================================"

# Step 7: Remove scheduled tasks
Write-Log "Step 6: Removing scheduled tasks..."
Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Scheduled Task Annihilator Has Started. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Scheduled Task Annihilator Has Started. Exit code: $exitCode"
Write-Host "================================================================"

Remove-ScheduledTasks

Write-Log "================================================================" -Level "INFO"
Write-Log "$AppName Scheduled Task Annihilator Has Finished. Exit code: $exitCode" -Level "INFO"
Write-Log "================================================================" -Level "INFO"
Write-Host "================================================================"
Write-Host "$AppName Scheduled Task Annihilator Has Finished. Exit code: $exitCode"
Write-Host "================================================================"

Write-Log "$AppName Removal Script Completed!" -Level "SUCCESS"
Write-Host "$AppName Removal Script Completed! `nRECOMMENDATION: Please restart your computer to ensure all changes take effect." -ForegroundColor Yellow
        """,
    ),
    "Wave Browser": UninstallerScript(
        name="Wave Browser",
        log_path=Path(r"C:\Temp\WavesorRemediationLog.txt"),
        powershell_code=r"""
...
Write-Host 'Script execution completed. Log file written to $LogFile'
    ...
    "Shift Browser": UninstallerScript(
        name="Shift Browser",
        log_path=Path(r"C:\ShiftBrowser_Removal_Log.txt"),
        powershell_code=r"""
...
Write-Log "Shift Browser cleanup process completed!"
    ...
    "OneLaunchOneStart": UninstallerScript(
        name="OneLaunchOneStart",
        log_path=Path(r"C:\OneStart_OneLaunch_Removal.log"),
...
if ($UninstallKeyFound) {
    ...
    if ($UninstallSuccess) {
        ...
        Write-Host "(SUCCESS) Graceful uninstall completed successfully!" -ForegroundColor Green
    } else {
        Write-Log "All graceful uninstall attempts FAILED. Proceeding with forced cleanup." -Level "ERROR"
        Write-Host "(FAILED) Graceful uninstall FAILED. Proceeding with forced cleanup." -ForegroundColor Red
    }
} else {
...
class UniversalUninstaller:
    ...
    def execute_powershell(self, script: UninstallerScript) -> ExecutionResult:
        ...
        Args:
            script: UninstallerScript containing the PowerShell code and configuration
            
        Returns:
            ExecutionResult with success status, log content, and any error messages
        """
        try:
            ...
            result = subprocess.run(
                [self.get_powershell_executable(), "-ExecutionPolicy", "Bypass", "-File", temp_path],
                capture_output=True,
                text=True,
                timeout=120,
            )
            ...
            log_content = ""
            log_file_exists = script.log_path.exists()
            
            if log_file_exists:
                log_content = script.log_path.read_text(encoding="utf-8")
                if log_content.strip():
                    self.logger.info(f"Log from {script.name}:\n{log_content}")

            # Success requires both PowerShell exit code 0 AND log file exists with content
            powershell_success = result.returncode == 0
            log_validation_success = log_file_exists and log_content.strip()
            overall_success = powershell_success and log_validation_success
            
            error_message = None
            if result.returncode != 0:
                error_message = result.stderr
            elif not log_file_exists:
                error_message = f"Log file not created at {script.log_path}"
            elif not log_content.strip():
                error_message = f"Log file created but is empty at {script.log_path}"
    ...
    def run_uninstallers(self, app_names: List[str]) -> BulkUninstallerResult:
        ...
        Args:
            app_names: List of application names to uninstall
            
        Returns:
            BulkUninstallerResult with overall status and individual script results
        """
        results = []

        self.logger.info(f"Running uninstallers for: {', '.join(app_names)}")
        ...
        bulk_result = BulkUninstallerResult(
        ...
        bulk_result.ticket_notes = self.generate_ticket_notes(bulk_result)
        return bulk_result
    ...
    def run_all_uninstallers(self) -> BulkUninstallerResult:
        ...
        all_log_paths = list(UNINSTALLER_SCRIPTS.get(app_name).log_path for app_name in all_app_names)
        self.logger.info(
            f"Running all available uninstallers: {', '.join(all_app_names)}"
        )
        self.logger.info(
            f"Log files will be saved to: {', '.join(str(path) for path in all_log_paths)}"
        )
        return self.run_uninstallers(all_app_names)
    
    def generate_ticket_notes(self, result: BulkUninstallerResult) -> str:
        """Generate detailed human-readable ticket notes summarizing the uninstall results.
        
        Args:
            result: BulkUninstallerResult containing the execution results
            
        Returns:
            Formatted string with comprehensive summary for ticket notes
        """
        lines = []
        lines.append("MALWARE UNINSTALL SUMMARY")
        lines.append("=" * 40)
        lines.append("")
        
        lines.append(f"Overall Status: {result.overall_status}")
        lines.append(f"Scripts Executed: {result.total_scripts}")
        lines.append(f"Successful: {result.successful_scripts}")
        lines.append(f"Failed: {result.failed_scripts}")
        lines.append("")
        ...
        for exec_result in result.results:
            status_icon = "(SUCCESS)" if exec_result.success else "(FAILED)"
            status_text = "SUCCESS" if exec_result.success else "FAILED"
            
            lines.append(f"{status_icon} {exec_result.script_name}: {status_text}")
            ...
            if exec_result.log_content:
                log_summary = exec_result.log_content.strip()
                if len(log_summary) > 200:
                    log_summary = log_summary[:200] + "..."
                if log_summary:
                    lines.append(f"   Log Preview: {log_summary}")
        ...
        if result.overall_status == "Success":
            lines.append("- All malware successfully removed")
            lines.append("- No further action required")
        elif result.overall_status == "Partial Success":
            failed_apps = [r.script_name for r in result.results if not r.success]
            lines.append(f"- {len(failed_apps)} application(s) require manual attention:")
            for app in failed_apps:
                lines.append(f"  - {app}")
            lines.append("- Review detailed logs in attachments")
        else:
            lines.append("- All uninstall attempts failed")
            lines.append("- Manual malware removal required")
            lines.append("- Escalate to security team if needed")
        
        lines.append("")
        lines.append("ATTACHMENTS:")
        lines.append("- Detailed PowerShell execution logs for each application")
        lines.append("- Review attachments for complete technical details")
...
def read_input_json():
    ...
    try:
        with open("input.json", "r", encoding="utf-8") as f:
            logger.info(json.dumps(json.load(f), indent=2, ensure_ascii=False))
    except Exception as e:
...
def main():
    ...
    Reads input from cw_rpa framework, executes appropriate uninstaller scripts,
    and logs results in the expected format. Supports:
    - List of specific app names to uninstall  
    - "all" keyword to run all available uninstallers
    - Single app name as string
    - Fallback to run all uninstallers if input format is unrecognized
    """
    try:
        input_provider = Input()
        uninstaller = UniversalUninstaller(logger=logger)
        
        read_input_json()

        try:
            app_names_input = input_provider.get_value(
                "AvailableApplications_1751478569946"
            )
            if not app_names_input:
                try:
                    app_names_input = input_provider.get_value("Applications_1750856640900")
                except KeyError:
                    app_names_input = "all"
        except Exception:
            app_names_input = "all"
        ...
        uninstaller.logger.result_data(
            {
                ...
                "ticket_notes": result.ticket_notes,
            }
        )
